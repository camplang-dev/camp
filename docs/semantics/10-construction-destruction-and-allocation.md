...
explicit within context or by implicit allocator forwarding when allowed by the
current allocation policy. `within` is not a call-argument modifier; write
`within (allocator) allocatingCall()` instead of
`allocatingCall(within allocator)`. If the current file requires explicit within
allocation, calls that need a within argument must diagnose instead of silently
inserting fallback.
