...
Use namespace blocks only when one file intentionally contains declarations
from more than one namespace. A block namespace replaces the file default; it
does not append to it. Use `namespace global { ... }` for root declarations in
a namespaced file and `global::Name` when code inside a namespace must qualify a
root declaration. For generated examples, prefer `PascalCase` namespace names
and lower camel case locals unless nearby code uses a different convention.
