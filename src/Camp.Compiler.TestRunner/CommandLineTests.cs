...
public sealed class CommandLineTests
{
	...
	public void Static_project_api_header_preserves_array_return_without_synthetic_length()
	{
		string source = CreateTempCase("static_api_array_return/library.camp", """
			namespace StaticApiArrayReturn;
			...
			""");
		string outDir = TempPath("static-api-array-return-out");
		...
		ProcessResult result = RunCampc(
			...
			outDir,
			"--name",
			"static_api_array_return");

		AssertCommandSucceeded(result);
		string apiPath = Path.Combine(outDir, ArtifactDirectoryForHost(NativeBuildKind.Static), "static_api_array_return_api.camp");
		Assert.True(File.Exists(apiPath), apiPath);
		string api = File.ReadAllText(apiPath);
		Assert.Contains("public extern const byte[] payload();", api, StringComparison.Ordinal);
		Assert.DoesNotContain("result_length", api, StringComparison.Ordinal);

		string consumer = CreateTempCase("static_api_array_return_consumer/main.camp", """
			using StaticApiArrayReturn;

			void callPayload()
			{
				const byte[] value = payload();
			}
			""");
		ProcessResult consumerResult = RunCampc(
			"build",
			...
			apiPath,
			"--out-dir",
			TempPath("static-api-array-return-consumer-out"),
			"--name",
			"static_api_array_return_consumer");
		AssertCommandSucceeded(consumerResult);
	}
	...
	[Fact]
	public void Transitive_project_reference_api_type_does_not_shadow_same_namespace_source_type_from_another_file()
	{
		string root = TempPath("transitive-api-type-cross-file-source-shadow");
		string foreignRoot = Path.Combine(root, "foreign");
		string bridgeRoot = Path.Combine(root, "bridge");
		string consumerRoot = Path.Combine(root, "consumer");
		Directory.CreateDirectory(foreignRoot);
		...
		File.WriteAllText(Path.Combine(consumerRoot, "types.camp"), """
			namespace Local;
		...
		File.WriteAllText(Path.Combine(consumerRoot, "consumer.camp"), """
			...
			public int readHidden(Thing value)
			{
				return value.hidden;
			}
			""".Replace("\r\n", "\n", StringComparison.Ordinal));
	...
	[Fact]
	public void Test_build_rebuilds_objects_when_private_header_changes()
	{
		string root = TempPath("test-private-header-incremental");
		string sourceRoot = Path.Combine(root, "src");
		Directory.CreateDirectory(sourceRoot);
		string producer = Path.Combine(sourceRoot, "producer.camp");
		string consumer = Path.Combine(sourceRoot, "consumer.camp");
		string project = Path.Combine(root, "app.campbuild");
		File.WriteAllText(project, """
			--name private-header-test-incremental
			src/*.camp
			""");
		File.WriteAllText(producer, """
			internal struct SharedData
		...
		File.WriteAllText(consumer, """
			...
			@test
			void readsSharedData(thrown Assertion* assertion)
			{
				SharedData data = makeData();
				assert(readData(data) == 9);
			}
			""");
		string target = NativeTargetForHost();

		ProcessResult firstRun = RunCampcIn(root, "test", "app.campbuild", "--target", target, "--out-dir", Path.Combine(root, "bin"));
		...
		ProcessResult secondRun = RunCampcIn(root, "test", "app.campbuild", "--target", target, "--out-dir", Path.Combine(root, "bin"), "--filter", "readsSharedData");
