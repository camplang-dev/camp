...
| Alias | Source |
|---|---|
| `spec` | `docs/camp_unified_spec_v38.md` |
...
| Construction/destruction is explicit: `init T(...)`, `new T(...)`, `delete valueOrPointer`, `expr finally delete`, and `expr finally cleanupMethod(args...)`. Use method cleanup for handle-like values that should not pretend cleanup is memory deallocation. | `CONFIRMED_BY_TEST` | `tests/CCompile/lifecycle_allocator.camp`; `tests/StdRun/pointer_new_array_finally_delete.camp`; `tests/StdRun/finally_cleanup_method.camp`; `src/Camp.Compiler/BindableNodeAnalyzer.Lowering.Operators.cs`; `src/Camp.Compiler/BindableNodeAnalyzer.Lowering.Exceptions.cs` |
...
| Method | Declared inside `struct`, `class`, `enum`, `newtype`, or `params` body. May use `: CallableNewtype` after the parameter list when the declaration family matches. Class/struct methods that implement interface slots must instead use `: Interface` or `: Interface.slotName`. If the callable newtype declares explicit callable `this`, omitted method `this` inherits those qualifiers. Methods declared inside `newtype` bodies use by-value implicit `this`; use an explicit `Type* this` extension method when pointer receiver semantics are required. | `CONFIRMED_BY_COMPILER_CODE` for ordinary methods; callable ascription is `SPEC_ONLY_OR_UNVERIFIED` | `src/Camp.Compiler/BindableNodeBuilder.cs::BuildStructDefinition`, `BuildClassDefinition`, `AddMethodOnlyScope`; `tests/CCompile/newtype_value_receivers.camp`; `tests/Diagnostics/interface_implementation_markers_invalid.camp`; `spec::1.4.16`, `1.11.6` |
| Static method | `static ReturnType name(params) [: CallableNewtype] { ... }` inside a type, or `static ReturnType Owner.name(params) body` at file scope for class/struct/newtype/primitive owners. Static methods are receiverless for callable ascription. Static members of generic types are accessed and declared as `Type.member`, not `Type<T>.member`, and cannot see enclosing type parameters; declare independent method type parameters when needed. | `CONFIRMED_BY_TEST` for ordinary static methods; callable ascription is `SPEC_ONLY_OR_UNVERIFIED` | `tests/CCompile/overload_basic.camp`; `tests/CCompile/generic_static_members.camp`; `tests/CCompile/out_of_scope_static_members.camp`; `tests/Diagnostics/generic_static_member_access_invalid.camp`; `tests/Diagnostics/out_of_scope_static_members_invalid.camp`; `src/Camp.Compiler/BindableNodeBuilder.cs::ApplyFunctionDeclarators`; `spec::1.4.16` |
...
| Field | `Type name;` or `static Type name;` inside `struct`/`class`; fixed-size array fields use `fixed T[n] name;`; type-scoped constants use `inline Type NAME = constant;`; file-scope static fields/constants may use `Owner.name` for class/struct/newtype/primitive owners. `escaped` is valid on fields to require escaped pointer-bearing assignments. `export`, `public`, and `extern` are valid only on explicit `static` fields, while `inline` is normalized to static constant behavior. Static and inline type fields of generic types are definition-level members accessed as `Type.member`, not `Type<T>.member`. Instance fields cannot use those visibility/import modifiers and cannot be `virtual`, `override`, `sealed`, `abstract`, or `async`. | `CONFIRMED_BY_TEST` | `tests/CCompile/fixed_array_storage.camp`; `tests/CCompile/inline_constants_fixed_enums.camp`; `tests/CCompile/lifetime_escaped_fields.camp`; `tests/Api/static_type_fields.camp`; `tests/Api/out_of_scope_static_members.camp`; `tests/CCompile/extern_class_opaque_helpers.camp`; `tests/CCompile/generic_static_members.camp`; `tests/CCompile/out_of_scope_static_members.camp`; `src/Camp.Compiler/BindableNodeBuilder.cs::ApplyFieldDeclarators`; `spec::1.6.8`, `2.2.1`, `4.1`, `5.1.8` |
...
| Interface | `[export|public|extern] interface Name[: interfaces] { methodSigs; }`. Ordinary interface methods may be required, optional with `= null`/`= default`, or defaulted with `= functionName`; optional/defaulted slots may be omitted by implementers. Implementing methods must be explicitly marked with `: Interface` or `: Interface.slotName`. | `CONFIRMED_BY_TEST` | `tests/CCompile/struct_interface_indirect.camp`; `tests/CCompile/overload_interface.camp`; `tests/CCompile/default_interface_methods.camp`; `tests/Diagnostics/interface_implementation_markers_invalid.camp` |
...
| Newtype value | `newtype Name: numericOrPointerType;`; body methods receive the `newtype` value by value. Value newtypes cannot declare constructors or destructors; use explicit cleanup methods plus `finally cleanup()` when needed. | `CONFIRMED_BY_TEST` | `tests/CCompile/newtype_value_receivers.camp`; `tests/Diagnostics/newtype_destructor_invalid.camp`; `src/Camp.Compiler/BindableNodeBuilder.cs::BuildNewtypeDefinition`, `IsValidValueNewtypeUnderlying`; `spec::1.11.5`, `1.11.6` |
...
| Value newtypes cannot declare destructors. For handle-like cleanup, declare an ordinary by-value instance method such as `close()`/`destroy()` and use `value finally close()` or call the method explicitly. | `CONFIRMED_BY_TEST` | `tests/Diagnostics/newtype_destructor_invalid.camp`; `tests/StdRun/finally_cleanup_method.camp`; `lib/std/src/std_file.camp`; `src/Camp.Compiler/BindableNodeBuilder.cs::AddNewtypeScope` |
...
| Finally cleanup expression | `expr finally delete` or `expr finally cleanupMethod(args...)` as expression suffixes. Cleanup methods must return `void`; cleanup method arguments are evaluated when cleanup runs. | `CONFIRMED_BY_TEST` | `tests/StdRun/pointer_new_array_finally_delete.camp`; `tests/StdRun/finally_cleanup_method.camp`; `src/Camp.Compiler/CampParser.cs::ParseUnaryExpression` |
...
| Methods that implement class/struct interface slots must be marked explicitly with `: Interface` for the matching callable name, or `: Interface.slotName` for a differently named implementation. Same-name methods without the marker do not implement required, optional, or defaulted slots. | `CONFIRMED_BY_TEST` | `tests/CCompile/default_interface_methods.camp`; `tests/CCompile/overload_interface.camp`; `tests/Diagnostics/interface_implementation_markers_invalid.camp`; `src/Camp.Compiler/BindableNodeAnalyzer.DeclarationValidation.cs::ValidateInterfaceImplementationMarkers` |
...
export static const HashPolicy<int> int.HASH_POLICY = { .hash = Int_hashcode, .equals = Int_hashEquals };
...
| Destructors | Ordinary struct/class destructors produce generated delete/destroy helpers. Value newtypes cannot declare destructors. Extern-class destructors are extern delete surfaces only; do not expect generated destroy/free logic. | `CONFIRMED_BY_TEST` | `tests/CCompile/extern_class_opaque_helpers.camp`; `tests/CCompile/extern_class_inheritance_lifecycle.camp`; `tests/Diagnostics/newtype_destructor_invalid.camp`; `src/Camp.Compiler/BindableNodeAnalyzer.Expansion.cs::CreateDeleteMethod`, `CreateDestroyMethod` |
...
| Interfaces | Interface implementation generates vtable storage, thunks, and indirect structs for structs. Only explicitly marked implementation methods fill vtable slots. | `CONFIRMED_BY_TEST` | `tests/CEmit/interface_vtable_exports.camp`; `tests/CCompile/struct_interface_indirect.camp`; `tests/Diagnostics/interface_implementation_markers_invalid.camp`; `src/Camp.Compiler/BindableNodeAnalyzer.Expansion.cs` |
...
31. Interfaces contain signatures only. Mark every implementing class/struct method explicitly with `: Interface` or `: Interface.slotName`; same-name methods do not implement slots by accident.
...
33. Use `init T(...)` for existing storage and `new T(...)` for allocation. Pair owned values/pointers with `delete` or `finally delete`; for handle-like value newtypes, use explicit cleanup methods such as `value finally close()`.
...
37. Pointer-form `delete` is valid only when the selected allocator `free` method or fallback `free(...)` accepts the pointer under ordinary type/lifetime conversion rules. `delete Newtype*` is pointer-storage deletion; it does not close or destroy the wrapped handle/value.
