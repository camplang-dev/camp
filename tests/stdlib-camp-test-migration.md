...
| `StdRun` | `console_streams` | stdlib-behavior | `dev/lib/std/tests/std_console_tests.camp` | partially-replaced | Console writer adapter availability is covered; legacy output-shape smoke remains active. |
| `StdRun` | `console_trimmed_line` | stdlib-behavior | `dev/lib/std/tests/std_console_tests.camp` | legacy-active | Keep legacy runtime output check active; no interactive terminal dependency is added to stdlib self-tests. |
| `StdRun` | `environment_filesystem_runtime` | stdlib-behavior | `dev/lib/std/tests/std_io_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves current directory, executable path, environment variables, directory lifecycle, copy/move/delete/size behavior. |
| `StdRun` | `file_errors` | stdlib-behavior | `dev/lib/std/tests/std_io_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves common missing-file error behavior. |
| `StdRun` | `file_handle` | stdlib-behavior | `dev/lib/std/tests/std_io_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves open/read/write/close byte behavior. |
| `StdRun` | `path_lexical_helpers` | stdlib-behavior | `dev/lib/std/tests/std_path_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves lexical path manipulation helpers. |
| `StdRun` | `time_functions` | stdlib-behavior | `dev/lib/std/tests/std_time_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves Date, TimeOfDay, DateTime, OffsetDateTime, Instant, TimeSpan, UtcOffset parse/format/arithmetic behavior. |
| `StdRun` | `timing_functions` | stdlib-behavior | `dev/lib/std/tests/std_timing_tests.camp` and `dev/lib/std/tests/std_atomic_tests.camp` | replaced | Dense replacement passed on macOS, Windows, and Linux; preserves sleep, sleepAsync, timers, cancellation, and atomic helper smoke behavior. |
